...
)
{
    ...
    if (TransA == CblasNoTrans && TransB == CblasNoTrans) {
        // Если две матрицы не требуют транспонирования
        for (m = 0; m < M; ++m) {
            for (n = 0; n < N; ++n) {
                sum = 0.0f;
        ...
        // В случае, когда матрицы требуют транспонирования
        for (m = 0; m < M; ++m) {
            for (n = 0; n < N; ++n) {
                sum = 0.0f;
                for (k = 0; k < K; ++k) {
                    a = (TransA == CblasNoTrans) ? A[m * lda + k] : A[k * lda + m];
                    b = (TransB == CblasNoTrans) ? B[k * ldb + n] : B[n * ldb + k];
                    sum += a * b;
                }
                C[m * ldc + n] = alpha * sum + beta * C[m * ldc + n];
            }
